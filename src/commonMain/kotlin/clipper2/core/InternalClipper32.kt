...
object InternalClipper32 {
    private const val MAXCOORD = Int.MAX_VALUE / 4
    private const val MAX_COORD = MAXCOORD.toDouble()
    private const val MIN_COORD = -MAXCOORD.toDouble()
    private const val Invalid32 = Int.MAX_VALUE
    ...
    fun crossProduct(pt1: Point32, pt2: Point32, pt3: Point32): Double {
        return ((pt2.x - pt1.x) * (pt3.y - pt2.y) - (pt2.y - pt1.y) * (pt3.x - pt2.x)).toDouble()
    }

    fun dotProduct(pt1: Point32, pt2: Point32, pt3: Point32): Double {
    ...
    fun checkCastInt32(v: Double): Int {
    ...
    fun getIntersectPt(
        ...
        ip.x = checkCastInt32((dx1 * qy - dx2 * qx) / cp)
        ip.y = checkCastInt32((dy1 * qy - dy2 * qx) / cp)
        return ip.x != Invalid32 && ip.y != Invalid32
    }
    ...
    fun getClosestPtOnSegment(offPt: Point32, seg1: Point32, seg2: Point32): Point32 {
        ...
        return Point32(seg1.x + (q * dx).roundToInt(), seg1.y + (q * dy).roundToInt())
    }
