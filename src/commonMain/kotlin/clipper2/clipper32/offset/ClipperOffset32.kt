...
class ClipperOffset32(
    ...
    fun addPath(path: Path32, joinType: JoinType?, endType: EndType?) {
        val cnt = path.size
        if (cnt == 0) {
            return
        }
    ...
    fun addPaths(paths: Paths32, joinType: JoinType?, endType: EndType?) {
        val cnt = paths.size
        if (cnt == 0) {
            return
        }
        _groupList.add(Group32(paths, joinType!!, endType!!))
    }
    ...
    fun execute(delta: Double, solution: Paths32) {
        solution.clear()
    ...
    fun execute(delta: Double, solution: PolyTree32) {
        solution.clear()
        executeInternal(delta)
    ...
    private fun getPerpendic(pt: Point32, norm: PointD): Point32 {
        return Point32(pt.x + norm.x * group_delta, pt.y + norm.y * group_delta)
    }

    private fun getPerpendicD(pt: Point32, norm: PointD): PointD {
        return PointD(pt.x + norm.x * group_delta, pt.y + norm.y * group_delta)
    }

    private fun doSquare(group: Group32, path: Path32, j: Int, k: Int) {
        val vec: PointD
        vec = if (j == k) {
        ...
        var ptQ = PointD(path[j].x.toDouble(), path[j].y.toDouble())
        ...
        if (j == k) {
            ...
            group.outPath.add(Point32(reflectPoint(pt, ptQ)))
            group.outPath.add(Point32(pt))
        } else {
    ...
    private fun doMiter(group: Group32, path: Path32, j: Int, k: Int, cosA: Double) {
    ...
    private fun doRound(group: Group32, path: Path32, j: Int, k: Int, angle: Double) {
        val pt = path[j]
    ...
    private fun buildNormals(path: Path32) {
        val cnt = path.size
        normals.clear()
    ...
    private fun offsetPoint(group: Group32, path: Path32, j: Int, k: RefObject<Int>) {
    ...
    private fun offsetPolygon(group: Group32, path: Path32) {
        group.outPath = Path32()
        val cnt = path.size
    ...
    private fun offsetOpenJoined(group: Group32, path: Path32) {
        var path = path
    ...
    private fun offsetOpenPath(group: Group32, path: Path32) {
        group.outPath = Path32()
    ...
    private fun doGroupOffset(group: Group32) {
        if (group.endType === EndType.Polygon) {
            // the lowermost polygon must be an outer polygon. So we can use that as the
            // designated orientation for outer polygons (needed for tidy-up clipping)
            val lowestIdx = OutObject<Int>(null)
            val grpBounds = OutObject<Rect32>(null)
...
//            }
        ...
        for (p in group.inPaths) {
            ...
            if (cnt == 1) {
                ...
                if (group.endType === EndType.Round) {
                    ...
                    val r = Rect32(path[0].x - d, path[0].y - d, path[0].x - d, path[0].y - d)
    ...
    companion object {
        private fun getUnitNormal(pt1: Point32, pt2: Point32): PointD {
        ...
        private fun getBoundsAndLowestPolyIdx(paths: Paths32, index: OutObject<Int>, recRef: OutObject<Rect32>) {
            val rec = Rect32(false) // ie invalid rect
            recRef.argValue = rec
            var lpX = Int.MIN_VALUE
            index.argValue = -1
