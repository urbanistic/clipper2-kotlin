 ...
 */
abstract class ClipperBase32 protected constructor() {
    ...
    private val minimaList: MutableList<LocalMinima32>
    ...
    private val scanlineSet: MutableList<Int>
    private val horzSegList: MutableList<HorzSegment>
    private val _horzJoinList: MutableList<HorzJoin>
    private var currentLocMin = 0
    private var currentBotY: Int = 0
    private var isSortedMinimaList = false
    private var hasOpenPaths = false
    var usingPolytree = false
    var succeeded = false

    class ClipperEngine32{
        companion object{
            private fun addLocMin(vert: Vertex?, polytype: PathType, isOpen: Boolean, minimaList: MutableList<LocalMinima32>) {
                // make sure the vertex is added only once ...
                if (vert!!.flags and VertexFlags.LocalMin != VertexFlags.None) {
                    return
                }
                vert.flags = vert.flags or VertexFlags.LocalMin
                val lm = LocalMinima32(vert, polytype, isOpen)
                minimaList.add(lm)
            }

            fun addPathsToVertexList(paths: Paths32, polytype: PathType, isOpen: Boolean, minimaList: MutableList<LocalMinima32>, vertexList: MutableList<Vertex?>) {
                for (path in paths) {
                    var v0: Vertex? = null
    ...
    class OutRec {
        ...
        var polypath: PolyPathBase32? = null
        var bounds = Rect32()
        var path = Path32()
        var isOpen = false
    ...
    class Active {
        var bot: Point32? = null
        var top: Point32? = null
        var curX: Int = 0 // current (updated at every new scanline)
        var dx = 0.0
        ...
        var localMin: LocalMinima32 = LocalMinima32() // the bottom of an edge 'bound' (also Vatti)
     ...
     */
    class OutPt(pt: Point32?, outrec: OutRec?) {
        var pt: Point32
     ...
     */
    internal class IntersectNode(val pt: Point32, val edge1: Active?, val edge2: Active?)
     ...
     */
    class Vertex(pt: Point32, flags: Int, prev: Vertex?) {
        var pt = Point32()
    ...
    init {
        minimaList = mutableListOf<LocalMinima32>()
    ...
    private fun insertScanline(y: Int) {
        scanlineSet.addIfMissingAndSort(y)
    }

    @Suppress("unused")
    @Deprecated("Has been inlined in Java version since function is much simpler")
    private fun popScanline(): Int {
    ...
    private fun hasLocMinAtY(y: Int): Boolean {
        return currentLocMin < minimaList.size && minimaList[currentLocMin].vertex!!.pt.y == y
    }

    private fun popLocalMinima(): LocalMinima32 {
     ...
     */
    open fun addSubjects(paths: Paths32) {
     ...
     */
    fun addOpenSubject(path: Path32) {
        addPath(path, PathType.Subject, true)
    }

    open fun addOpenSubjects(paths: Paths32) {
     ...
     */
    fun addClip(path: Path32) {
        addPath(path, PathType.Clip)
    }

    fun addClips(paths: Paths32) {
        addPaths(paths, PathType.Clip)
    }

    open fun addPath(path: Path32, polytype: PathType, isOpen: Boolean = false) {
        val tmp = Paths32()
        tmp.add(path)
        addPaths(tmp, polytype, isOpen)
    }

    open fun addPaths(paths: Paths32, polytype: PathType, isOpen: Boolean = false) {
        if (isOpen) {
            hasOpenPaths = true
        }
        isSortedMinimaList = false
        ClipperEngine32.addPathsToVertexList(paths, polytype, isOpen, minimaList, vertexList)
    }

    fun addReuseableData(reuseableData: ReuseableDataContainer32)
    {
        ...
        {
            minimaList.add(LocalMinima32(lm.vertex, lm.polytype, lm.isOpen))
    ...
    private fun insertLocalMinimaIntoAEL(botY: Int) {
        var localMinima: LocalMinima32
    ...
    private fun addLocalMinPoly(ae1: Active?, ae2: Active?, pt: Point32?, isNew: Boolean = false): OutPt {
    ...
    private fun addLocalMaxPoly(ae1: Active, ae2: Active, pt: Point32): OutPt? {
        if (isJoined(ae1)) {
            split(ae1, pt)
        }
        if (isJoined(ae2)) {
    ...
    private fun startOpenPath(ae: Active?, pt: Point32?): OutPt {
        val outrec = newOutRec()
        outrec.isOpen = true
        if (ae!!.windDx > 0) {
            outrec.frontEdge = ae
    ...
    private fun adjustCurrXAndCopyToSEL(topY: Int) {
        var ae = actives
        sel = ae
        while (ae != null) {
    ...
    protected fun executeInternal(ct: ClipType, fillRule: FillRule) {
        ...
        var y: Int = scanlineSet.pollLast()
        while (succeeded) {
    ...
    private fun doIntersections(topY: Int) {
    ...
    private fun addNewIntersectNode(ae1: Active?, ae2: Active?, topY: Int) {
        var ip = Point32()
        if (!getIntersectPt(ae1!!.bot!!, ae1.top!!, ae2!!.bot!!, ae2.top!!, ip)) {
            ip = Point32(ae1.curX, topY)
        }
    ...
    private fun buildIntersectList(topY: Int): Boolean {
	 ...
	 *******************************************************************************/ {
        ...
        var leftX: Int
        val tempOutleftX = OutObject<Int>(null)
        var rightX: Int
        val tempOutrightX = OutObject<Int>(null)
        ...
        while (true) {
            ...
            val tempOutleftX2 = OutObject<Int>(null)
            val tempOutrightX2 = OutObject<Int>(null)
    ...
    private fun doTopOfScanbeam(y: Int) {
    ...
    private fun split(e: Active?, currPt: Point32?) {
    ...
    protected fun buildPaths(solutionClosed: Paths32, solutionOpen: Paths32): Boolean {
    ...
    protected fun buildTree(polytree: PolyPathBase32, solutionOpen: Paths32) {
        polytree.clear()
        solutionOpen.clear()
        var i = 0
        // _outrecList.Count is not static here because
    ...
    fun getBounds(): Rect32 {
        val bounds = Clipper.InvalidRect32
        for (t in vertexList) {
            var v = t
            do {
        ...
        return if (bounds.isEmpty()) Rect32(0, 0, 0, 0) else bounds
    }
    ...
    companion object {
        ...
        private fun getDx(pt1: Point32, pt2: Point32): Double {
            /*-
		 *  Dx:                             0(90deg)                                    *
		 *                                  |                                           *
        ...
        private fun topX(ae: Active, currentY: Int): Int {
            ...
            return if (currentY == ae.bot!!.y) {
                ...
                ae.bot!!.x + (ae.dx * (currentY - ae.bot!!.y)).roundToInt()
            }
        }
        ...
        private fun areaTriangle(pt1: Point32, pt2: Point32, pt3: Point32): Double {
        ...
        private fun addOutPt(ae: Active, pt: Point32): OutPt {
        ...
        private fun getBounds(op: OutPt): Rect32 {
            val result = Rect32(op.pt.x, op.pt.y, op.pt.x, op.pt.y)
            var op2 = op.next
            while (op2 !== op) {
        ...
        private fun pointInOpPolygon(pt: Point32, op: OutPt): PointInPolygonResult {
            var op = op
        ...
        private fun ptsReallyClose(pt1: Point32, pt2: Point32): Boolean {
        ...
        fun buildPath(op: OutPt?, reverse: Boolean, isOpen: Boolean, path: Path32): Boolean {
            var op = op
        ...
        fun getBounds(path: Path32): Rect32 {
            ...
            val result = Clipper.InvalidRect32
            for (pt in path) {
                if (pt.x < result.left) {
                    result.left = pt.x
                }
